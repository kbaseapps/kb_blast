...
WORKDIR /kb/module
RUN \
  curl ftp://ftp.ncbi.nlm.nih.gov/blast/executables/blast+/LATEST/ncbi-blast-2.10.1+-x64-linux.tar.gz > ncbi-blast-2.10.1+-x64-linux.tar.gz && \
  tar xfz ncbi-blast-2.10.1+-x64-linux.tar.gz && \
  ln -s ncbi-blast-2.10.1+ blast && \
  rm -f ncbi-blast-2.10.1+-x64-linux.tar.gz && \
