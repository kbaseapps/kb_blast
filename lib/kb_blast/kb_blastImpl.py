...
#        report += "\n"+pformat(params)
        ...
        #
        if not bu.validate_BLAST_app_params (params, method_name):
        ...
        if 'input_one_sequence' in params \
            ...
            input_one_ref = bu.objectify_text_query (params, 'nucleotide', method_name)
        ...
        #feature_ids = target_feature_info['feature_ids']
        #feature_ids_by_genome_ref = target_feature_info['feature_ids_by_genome_ref']
        #feature_ids_by_genome_id = target_feature_info['feature_ids_by_genome_id']
        #feature_id_to_function = target_feature_info['feature_id_to_function']
        #genome_ref_to_sci_name = target_feature_info['genome_ref_to_sci_name']
        ...
        if len(invalid_msgs) > 0:
            error_report_info = bu.save_error_report_with_invalid_msgs (invalid_msgs, input_one_ref, input_many_ref, method_name)
        ...
        #### FORMAT DB
        ##
        if not bu.format_BLAST_db (search_tool_name, target_fasta_file_path):
            raise ValueError ("failed to format BLAST db")
            

        #### Run BLAST for base format
        ##
        BLAST_output_results = bu.run_BLAST (search_tool_name = search_tool_name, 
        ...
        if str(params.get('output_extra_format')) and str(params.get('output_extra_format')) != 'none':
            ...
            BLAST_extra_output_results = bu.run_BLAST (search_tool_name = search_tool_name, 
                                                       query_fasta_file_path = query_fasta_file_path, 
                                                       target_fasta_file_path = target_fasta_file_path, 
                                                       e_value = str(params['e_value']),
                                                       maxaccepts = str(params['maxaccepts']),
        ...
        #
        query_len = bu.get_query_len (query_fasta_file_path)


        # Parse the BLAST tabular output and store ids to filter many set to make filtered object to save back to KBase
        #
        parsed_BLAST_results = bu.parse_BLAST_tab_output (output_aln_file_path = output_aln_file_path,
        ...
        #
        if len(invalid_msgs) == 0:
            report_info = bu.build_BLAST_report (search_tool_name = search_tool_name,
                                                 params = params,
                                                 target_type_name = target_type_name,
                                                 target_feature_info = target_feature_info,
