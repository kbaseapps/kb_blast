...
** Initially only basic query/db search will be implemented between read sets
    ...
    def BLASTn_Search(self, ctx, params):
        ...
        # Parse the BLAST tabulard output and store ids to filter many set to make filtered object to save back to KBase
        #
                ...
                for genomeRef in genome2Features:
                    ...
                    for feature in genome['features']:
                        if feature['id'] in these_genomeFeatureIds:
                            #record = SeqRecord(Seq(feature['dna_sequence']), id=feature['id'], description=genomeRef+"."+feature['id'])
                            if feature['type'] != 'CDS':
                                raise ValueError("bad feature type "+feature['type']+" for feature ID "+feature['id'])
                            else:
